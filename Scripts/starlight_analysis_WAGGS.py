...
class StarlightOutput:
    ...
    def read_file(this):
        ...
        for i, line in enumerate(lines):
            ...
            if "[chi2/Nl_eff]" in line:
                try:
                    this.chi2 = float(line.split()[0])
                except:
                    this.chi2 = np.nan
            elif "[adev (%)]" in line:
                try:
                    this.adev = float(line.split()[0])
                except:
                    this.adev = np.nan
            elif "[AV_min" in line:
                try:
                    this.av = float(line.split()[0])
                except:
                    this.av = np.nan
            elif "[v0_min" in line:
                try:
                    this.v0 = float(line.split()[0])
                except:
                    this.v0 = np.nan
            elif "[vd_min" in line:
                try:
                    this.vd = float(line.split()[0])
                except:
                    this.vd = np.nan
            elif "[NOl_eff]" in line:
                try:
                    this.n0 = int(line.split()[0])
                except:
                    this.n0 = np.nan
            elif "[Nl_eff]" in line:
                try:
                    this.nl = int(line.split()[0])
                except:
                    this.nl = np.nan
            elif "[Ntot_cliped & clip_method]" in line:
                try:
                    this.nclip = int(line.split()[0])
                except:
                    this.nclip = np.nan
            elif "[arq_base]" in line:
                try:
                    this.base = line.split()[0]
                except:
                    this.base = None
            ...
            if reading_population:
                ...
                if len(line) == 0 or line.startswith("##"):
                    ...
                    try:
                        ...
                        this.population["x_j"].append(
                            float(cols[1])
                        )  # Fração de luz [%]
                        this.population["m_ini"].append(
                            float(cols[2])
                        )  # Fração de massa inicial [%]
                        this.population["m_cor"].append(
                            float(cols[3])
                        )  # Fração de massa atual [%]
        ...
        this.min_lambda = (
            np.min(this.spectrum["l_obs"]) if len(this.spectrum["l_obs"]) > 0 else None
        )
        this.max_lambda = (
            np.max(this.spectrum["l_obs"]) if len(this.spectrum["l_obs"]) > 0 else None
        )
    ...
    def calculate_mean_properties(this):
        ...
        return {
            ...
            "mean_age_light_gyr": (10**mean_log_age_light)
            / 1e9,  # Converte log(anos) para Giga-anos
    ...
    def plot_fit(this, save_fig=True):
        """
        ...
        if this.chi2 is not None:
            title_str += f"$\chi^2/N_{{eff}}={this.chi2:.2f}$  "
        if this.adev is not None:
            title_str += f"Adev={this.adev:.2f}%  "
        if this.av is not None:
            title_str += f"$A_V={this.av:.2f}$ mag"
